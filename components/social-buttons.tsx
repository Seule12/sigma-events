"use client";

const PROVIDERS = [
  {
    id: "google",
    label: "Google",
    icon: (
      <svg className="h-4.5 w-4.5" viewBox="0 0 24 24" aria-hidden="true">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.27-4.74 3.27-8.1z" />
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
        <path fill="#FBBC05" d="M5.84 14.1c-.22-.66-.35-1.36-.35-2.1s.13-1.44.35-2.1V7.06H2.18A10.96 10.96 0 0 0 1 12c0 1.77.43 3.45 1.18 4.94l3.66-2.84z" />
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z" />
      </svg>
    ),
  },
  {
    id: "facebook",
    label: "Facebook",
    icon: (
      <svg className="h-4.5 w-4.5" viewBox="0 0 24 24" fill="#1877F2" aria-hidden="true">
        <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073C0 18.1 4.388 23.094 10.125 24v-8.437H7.078v-3.49h3.047v-2.66c0-3.026 1.792-4.697 4.533-4.697 1.313 0 2.686.236 2.686.236v2.971H15.83c-1.491 0-1.956.93-1.956 1.886v2.264h3.328l-.532 3.49h-2.796V24C19.612 23.094 24 18.1 24 12.073z" />
      </svg>
    ),
  },

] as const;

export default function SocialButtons({ enabled }: { enabled: string[] }) {
  return (
    <div className="space-y-2.5">
      {PROVIDERS.map((p) => {
        const isEnabled = enabled.includes(p.id);
        const cls = `flex w-full items-center justify-center gap-3 rounded-xl border px-4 py-3 text-sm font-bold transition ${
          isEnabled
            ? "border-white/10 bg-white/5 text-slate-200 backdrop-blur-xl hover:border-brand-400/50 hover:bg-white/10 hover:text-white active:scale-[0.98]"
            : "cursor-not-allowed border-white/5 bg-white/[0.03] text-slate-600"
        }`;
        return isEnabled ? (
          <a key={p.id} href={`/api/auth/${p.id}`} className={cls}>
            {p.icon}
            <span>Continuer avec {p.label}</span>
          </a>
        ) : (
          <span key={p.id} className={cls} title="Configuration en cours">
            {p.icon}
            <span>Bientôt disponible</span>
          </span>
        );
      })}
    </div>
  );
}
